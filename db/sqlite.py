...
class Db:
    ...
    def add_leaf(self, link_parent_id, string_id, target_ino):
        self.__conn.execute('INSERT INTO tree (parent_id, name, inode) VALUES (?, ?, ?)',
    ...
    def list_childs(self, node_id):
        return self.__conn.execute(
            'SELECT t.inode, s.value FROM tree t, strings s WHERE t.parent_id = ? AND t.name = s.id',
            (node_id,)).fetchall()
    ...
    def add_hash_to_index(self, inode, hash_id, block_nr):
        self.__conn.execute('INSERT INTO "index" (inode, hash_id, block_nr) VALUES (?, ?, ?)',
    ...
    def get_used_space(self):
        return \
        self.__conn.execute('SELECT SUM(inodes.size) FROM tree, inodes WHERE tree.inode = inodes.inode').fetchone()[0]


    def get_disk_usage(self):
        return self.__conn.execute('PRAGMA page_size').fetchone()[0] * \
               self.__conn.execute('PRAGMA page_count').fetchone()[0]
